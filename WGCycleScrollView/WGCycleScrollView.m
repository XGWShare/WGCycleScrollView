...
@interface WGCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation WGCycleScrollView
...
- (void)initialization
{
    _pageControlAliment = WGCycleScrollViewPageContolAlimentCenter;
    ...
    _pageControlStyle = WGCycleScrollViewPageContolStyleClassic;
...
{
    WGCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setupMainView
{
    ...
    [mainView registerClass:[WGCollectionViewCell class] forCellWithReuseIdentifier:ID];
...
- (void)setPageControlStyle:(WGCycleScrollViewPageContolStyle)pageControlStyle
{
...
- (void)loadImageAtIndex:(NSInteger)index
{
    ...
    // 如果有缓存，直接加载缓存
    NSData *data = [_cacheImage objectForKey:urlStr];
    if (data) {
        ...
        NSURLSessionDataTask *asyncDataTask = [session dataTaskWithRequest:[NSURLRequest requestWithURL:url] completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (!error) {
                ...
                [_cacheImage setObject:data forKey:url.absoluteString];
            } else { // 加载数据失败
                static int repeat = 0;
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        ...
        case WGCycleScrollViewPageContolStyleClassic:
        {
...
- (void)layoutSubviews
{
    ...
    if (self.pageControlAliment == WGCycleScrollViewPageContolAlimentRight) {
...
{
    WGCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
